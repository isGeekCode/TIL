...
Dart에서는 변수 선언 시 타입을 명시하거나 생략할 수 있다.
실무에서는 가독성과 안정성을 위해 명시적 선언을 권장한다.
...
## 🔍 컴파일 시점과 런타임 시점
- 컴파일 시점 (Compile Time)
    - 코드 실행 전, Dart 컴파일러가 코드를 분석하고 오류를 검사하는 단계.
    - const는 반드시 이 시점에서 값이 확정되어야 한다.
- 런타임 시점 (Run Time)
    - 실제 앱 실행 중인 단계. 사용자 입력, 현재 시간, 네트워크 응답 등은 런타임에서만 알 수 있다.
    - final은 런타임 값 할당 가능하다.
...
```dart
void main() {
  var age = 20;            // 타입 추론 → int
  dynamic anything = "Hi"; // 어떤 타입이든 가능
  final birthYear = 2000;  // 실행 중에 결정 가능
  const pi = 3.14;         // 반드시 컴파일 시점에 확정
}
```
...
num numValue = 10;            // int/double 상위 타입
